        ...
        <v-card flat class="mr-2 mt-2 ml-1" :href="/restaurant/ + restaurantId">
            ...
            <v-card-actions class="py-2 px-1">
                <v-container class="px-1 py-0">
                    <v-layout>
                        <v-img :src="require('@/assets/'+restaurant.primary_picture)" aspect-ratio="2" width="200px"></v-img>
                    </v-layout>
                    <v-layout align-center row class="pt-1">
                        <span class="font-weight-thin grey--text subheading  text--darken-1">Types: </span>
                        <v-chip outline small v-for="(type) in restaurantTypes" :key="type">{{type}}</v-chip>
                    </v-layout>
                    <v-layout align-center row class="pt-1">
                        <span class="font-weight-thin grey--text subheading  text--darken-1">Rating: </span>
...
export default {
    ...
    methods: {
        ...
        getRestaurantById(id) {
            let restaurant = this.restaurantsJson.restaurants.filter(obj => obj.id.match(id))

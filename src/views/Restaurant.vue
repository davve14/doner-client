                ...
                <v-flex>
                    <v-card flat>
                        <v-carousel flat height="400">
                            <v-carousel-item v-for="(picture,i) in restaurant.secondary_pictures" :key="i" :src="require('@/assets/'+picture)"></v-carousel-item>
                        </v-carousel>
                    </v-card>
                </v-flex>
                <v-flex>
                    <v-card flat class="mt-3">
                        <mapbox access-token="pk.eyJ1IjoiZGF2dmUxNCIsImEiOiJjanFpZjVnYXgwNGF2NDJueHQzZGpoZ3IwIn0.nb0lYF98Ue-7Xt94vgXWkg"
                            :map-options="{
    ...
    methods: {
        ...
        getRestaurant() {
            let filtered = this.restaurants.filter((obj) => {
                return obj.id.match(this.$route.params.id)
            })
